...
languages.GEN_POSTCANT_1 = function()


    local words_end_in_vowel_chance = 25 

    local common_vowel = { "u", "o" }
    local medium_vowel = { "a" }
    local rare_vowel = { "i", "e" }

    local common_vowel_pair = { "ou", "au" }
    local medium_vowel_pair = { "uo", "ua" }
    local rare_vowel_pair = { "gu", "vu" }

    local common_consonant = { "v", "g", "k", "r", "m", "d", "b" }
    local medium_consonant = { "n", "l", "z", "t", "p" }
    local rare_consonant = { "s", "sh", "h" }

    local common_first_only = { "gr", "dr", "kr", "br" }
    local medium_first_only = { "gv", "kv", "vl" }
    local rare_first_only = {}

    local common_middle_only = { "gg", "kk", "vv", "rr" }
    local medium_middle_only = { "rg", "rk", "rv", "dr" }
    local rare_middle_only = { "nd", "mb" }

    local common_end_only_consonant = {"g", "k", "v", "r"}
    local medium_end_only_consonant = {"d", "b", "m"}
    local rare_end_only_consonant = {"t"}
...
local function has_valid_doubles(word)
    ...
    for k, v in ipairs(world.language.word) do
        ...
        gen_postcant1[v.token] = str
    end

    return gen_postcant1
end
