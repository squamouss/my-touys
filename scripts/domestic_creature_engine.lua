...
function generate_domestic_creatures_for_civs(generated_civs, domestic_rules)
    ...
    for _, civ_info in ipairs(generated_civs) do
        ...
        local chosen_rule = pick_random(potential_config_rules)
        ...
        for rating_tag, override in pairs(rating_overrides) do
            ...
            if override.min and override.max then
                ...
                if potential_extras > 0 then
                    -- Check if the rule has specific probability params (optional feature for future)
                    -- otherwise use defaults:
                    -- base_chance: Probability to get the 1st extra creature (e.g., 80%)
                    -- decay_factor: Multiplier applied to chance after each success (e.g., 0.6)
