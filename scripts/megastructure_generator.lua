...
local function run_megastructure_gen()
    ...
    local function create_raw_object(token, def, resource_list, nm)
        ...
        if nm and nm > 0 then total_yield = total_yield + nm end
    ...
    local function generate_cycle(index, force_def_name, source_table)
        ...
        if success then
            ...
            table.insert(generated_objects, create_raw_object("MEGASTRUCT_GEN_" .. index .. "_" .. pct_ma .. "_" .. pct_ec, def, resources, pct_nm))
        end
    end

    -- Initial generation
    for i = 1, NUM_TO_GENERATE do generate_cycle(i, nil, GENERATION_POOL) end
    
    -- Guarantee Pass
    local extra_idx = NUM_TO_GENERATE + 1
    for _, def in ipairs(MAIN_MATERIALS) do
        if not generated_types[def.name] then generate_cycle(extra_idx, def.name, MAIN_MATERIALS); extra_idx = extra_idx + 1 end
    end
    for _, def in ipairs(GEM_MATERIALS) do
        if not generated_types[def.name] then generate_cycle(extra_idx, def.name, GEM_MATERIALS); extra_idx = extra_idx + 1 end
    end
    ...
    -- Execute registration in hierarchical order
    if _G.raws and _G.raws.register_inorganics then
        ...
        if #layer_lines > 0 then
            ...
            if _G.log then _G.log("MEGASTRUCT_LOG: Registered Layer lines (Pass 1).") end
        end

        -- Step 2: Register Inclusions/Gems (Children)
        if #inclusion_lines > 0 then
            _G.raws.register_inorganics(inclusion_lines)
            if _G.log then _G.log("MEGASTRUCT_LOG: Registered Inclusion lines (Pass 2).") end
        end
    end
end
