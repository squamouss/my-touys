...
-- Used by Master Hook for [DIFFICULTY:X] token
...
-- Used by Master Hook for Description Text
function M.get_description_string(config, components)
    local score = M.calculate_difficulty(config, components)
    return " THREAT_RANK= " .. tostring(score) .. "/"
end
