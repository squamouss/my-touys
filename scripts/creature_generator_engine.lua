...
local function select_random_creature_components(config, preferred_ratings_pool, valid_sub_archetypes, forced_size_tag)
    ...
    -- [[ FIX START: Robust Size Selection (Strings, Tables, and "any") ]] --
    -- First, get ALL compatible sizes for this rating and sub-archetype
    ...
    -- If a forced size is requested, filter the pool
    if forced_size_tag then
        
        -- CASE 1: "any" - Skip filtering, accept all compatible sizes
        if type(forced_size_tag) == "string" and forced_size_tag:lower() == "any" then
            ...
            for _, sz in ipairs(compatible_sizes) do
                ...
                -- CASE 2: Table of Strings (e.g. {"MDM", "LRG"})
                ...
                -- CASE 3: Single String (e.g. "GIANT")
                else
            ...
            -- If we found valid sizes for the force, use them. 
            -- Otherwise, fall back to the generic pool (ignore force) to prevent crash.
...
local function generate_all_creatures(config)
    ...
    -- NOTE: fingerprints is replaced by _G.global_creature_fingerprints for validation
    -- but we can still use a local one for batch uniqueness if needed, but strict global is safer.
    
    local max_attempts = num_to_generate * 200
    -- [[ NEW: Strict limit for experiments ]]
    if config.experiment_context then
        max_attempts = 10
    end
    ...
    while successfully_generated_count < num_to_generate and max_attempts > 0 do
        ...
        if selection_successful and components then
            ...
            -- [[ UPDATED: Check Global Registry for Uniqueness ]]
            ...
            if is_unique then
                ...
                -- Duplicate found logic
                if config.experiment_context then
                    -- Just loop again until max_attempts runs out
                end
            end
        else
...
if arg and arg[1] then
    local function add_current_dir_to_path()
        ...
        if info and info.source then 
            ...
            if script_path then
                -- Add the script's directory AND the scripts subdirectory just in case
    ...
    if status_wep and status_cfg and weaponizer and weapon_config then
        ...
        for i, obj in ipairs(generated_data) do
             -- modify the 'creature' field (raw lines) in place
        ...
        -- Debugging print to help if it still fails
