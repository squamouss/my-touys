...
local addon_master = require("creature_addon_hook_master") -- [[ CHANGED: Centralized Hook ]]
...
local function construct_full_description(config, comps, final_mass, additional_line, include_rating_desc, caste_type)
    ...
    -- [[ CHANGED: Use Master Hook for Description Addons ]] --
    if addon_master then
        addon_master.append_description_info(parts, config, comps)
    end
    -- [[ END CHANGED ]] --
...
local function assemble_creature_raw(config, creature_id_suffix, components, numeric_code)
    ...
    -- [[ MOVED: Sphere Rules Injection (Handled via Addon Master now if configured) ]]
    -- We can rely on the addon master loop later, or call it here if ordering is vital.
    -- However, the original code had it here. For safety, the addon master will append tokens later.
    -- The ordering change should be fine as tokens generally apply regardless of order, 
    -- but if you need them before caste generation, consider this. 
    -- For now, we will let the Master Hook handle it at the end of the list where "extensions" go.
    ...
    -- [[ CHANGED: Use Master Hook for Token Addons ]] --
    if addon_master then
        local addon_tokens = addon_master.collect_tokens(config, components)
        split_to_lines(lines, addon_tokens)
    end
    -- [[ END CHANGED ]] --
    ...
    return { creature=lines, weight=10, is_transcendent=false, components=components }
end
