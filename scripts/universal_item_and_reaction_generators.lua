...
local function generate_assets(item_id_suffix, item_name, item_tile, reaction_name, category_id, category_name, category_desc, all_syndrome_blocks, is_first_in_category, fortress_reagent_line, adventure_reagent_line)
    local inorganic_raws = {}
    local reaction_raws = {}
    local inorganic_id = "TRANSFORM_ITEM_" .. item_id_suffix

    table.insert(inorganic_raws, "[INORGANIC:" .. inorganic_id .. "]")
    _G.add_generated_info(inorganic_raws)
    table.insert(inorganic_raws, "[USE_MATERIAL_TEMPLATE:MUSCLE_TABLET_TEMPLATE][MEAT][SPECIAL]")
    table.insert(inorganic_raws, "[STATE_NAME_ADJ:ALL_SOLID:" .. item_name .. "][DISPLAY_COLOR:7:11:0][TILE:'" .. item_tile .. "']")
    table.insert(inorganic_raws, "[SOLID_DENSITY:500][GENERATED]")
    table.insert(inorganic_raws, "[STATE_COLOR:ALL_SOLID:MAUVE]")
    table.insert(inorganic_raws, "[MEAT_NAME:NONE:" .. item_name .. ":" .. item_name .. "s]")
    ...
    for _, syndrome_block in ipairs(all_syndrome_blocks) do
        ...
        for line in reformatted_block:gmatch("[^\r\n]+") do
            table.insert(inorganic_raws, line)
        end
    end

    -- [[ DEBUG LOG TOGGLE FOR INORGANIC ]] --
    if ENABLE_RAW_LOGGING then
        table.insert(inorganic_raws, "[LOG_CURRENT_ENTRY]")
    end
    -- [[ END DEBUG LOG ]] --

    -- Fortress Reaction
    table.insert(reaction_raws, "[REACTION:FORTRESS_" .. item_id_suffix .. "]")
    table.insert(reaction_raws, "[NAME:" .. reaction_name .. "]")
    table.insert(reaction_raws, "[BUILDING:N_MATTER_REFINERY:NONE][FORTRESS_MODE_ENABLED][GENERATED]")
    if not DEBUG_FREE_REACTIONS then
        table.insert(reaction_raws, fortress_reagent_line)
    end
    table.insert(reaction_raws, "[PRODUCT:100:1:MEAT:NONE:INORGANIC:" .. inorganic_id .. "]")
    table.insert(reaction_raws, "[SKILL:EXTRACT_STRAND]")
    table.insert(reaction_raws, "[CATEGORY:" .. category_id .. "_FORT]")
    if is_first_in_category then
        table.insert(reaction_raws, "[CATEGORY_NAME:" .. category_name .. "]")
        table.insert(reaction_raws, "[DESCRIPTION:" .. category_desc .. "]")
    end
    
    -- [[ DEBUG LOG TOGGLE FOR FORTRESS REACTION ]] --
    if ENABLE_RAW_LOGGING then
        table.insert(reaction_raws, "[LOG_CURRENT_ENTRY]")
    end
    -- [[ END DEBUG LOG ]] --

    -- Adventure Reaction
    table.insert(reaction_raws, "[REACTION:ADVENTURE_" .. item_id_suffix .. "]")
    table.insert(reaction_raws, "[NAME:" .. reaction_name .. "]")
    table.insert(reaction_raws, "[ADVENTURE_MODE_ENABLED][GENERATED]")
    if not DEBUG_FREE_REACTIONS then
        table.insert(reaction_raws, adventure_reagent_line)
        table.insert(reaction_raws, "[REAGENT:B:1:TOOL:ITEM_TOOL_PORTABLE_SYNTH:NONE:NONE][PRESERVE_REAGENT]")
    end
    table.insert(reaction_raws, "[PRODUCT:100:1:MEAT:NONE:INORGANIC:" .. inorganic_id .. "]")
    table.insert(reaction_raws, "[SKILL:EXTRACT_STRAND]")
    table.insert(reaction_raws, "[CATEGORY:" .. category_id .. "_ADV]")
    if is_first_in_category then
        table.insert(reaction_raws, "[CATEGORY_NAME:" .. category_name .. "]")
        table.insert(reaction_raws, "[CATEGORY_DESCRIPTION:" .. category_desc .. "]")
    end

    -- [[ DEBUG LOG TOGGLE FOR ADVENTURE REACTION ]] --
    if ENABLE_RAW_LOGGING then
        table.insert(reaction_raws, "[LOG_CURRENT_ENTRY]")
    end
    -- [[ END DEBUG LOG ]] --

    _G.raws.register_inorganics(inorganic_raws)
    _G.raws.register_reactions(reaction_raws)
end
...
local function generate_all_transformation_items()
    script_log("ITEM_GEN_LOG: Starting generation (V5.6 Name Parsing Fix).")
    ...
    script_log("ITEM_GEN_LOG: Finished generation. Specific Items: " .. specific_generated_count .. ".")
end
