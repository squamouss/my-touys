...
local function sort_loadout_by_priority(loadout)
    table.sort(loadout, function(a, b)
        -- Lower number = Higher priority (First in list)
        -- Default to 999 if no priority is set so they appear last
        local p1 = a.priority or 999
        local p2 = b.priority or 999
        
        if p1 == p2 then
            -- Secondary sort by key to ensure deterministic order if priorities match
...
function M.apply_weapons(raw_lines, weapon_config, forced_theme)
    ...
    if forced_theme then
        ...
        if theme_buckets[mapped_theme] and #theme_buckets[mapped_theme] > 0 then
            ...
            if #available_themes > 0 then
                ...
                local idx
                if _G.trandom then
                    idx = rand_func(#available_themes) + 1
                else
                    idx = rand_func(#available_themes)
                end
    ...
    if theme_buckets[selected_theme] then
        for _, w in ipairs(theme_buckets[selected_theme]) do table.insert(eligible_weapons, w) end
    end
    if selected_theme ~= "UNIVERSAL" and theme_buckets["UNIVERSAL"] then
        for _, w in ipairs(theme_buckets["UNIVERSAL"]) do table.insert(eligible_weapons, w) end
    end
    ...
    local function recursive_combine(group_idx, current_loadout)
        if group_idx > #group_names then
            ...
            for _, w in ipairs(standalone) do table.insert(final, w) end
    ...
    recursive_combine(1, {})
    if #groups == 0 and #standalone > 0 then potential_loadouts = { standalone } end
